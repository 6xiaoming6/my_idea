...
from .transforms import ensure_multiscale, prepare_single_scale, to_bcthw
from .diverse_masks import DiverseMaskSchedule
...
class FlowNPZDataset(Dataset):
    ...
    def __init__(
        ...
        diverse_mask_config: dict | None = None,
    ) -> None:
        self.path = Path(path)
        with np.load(self.path, allow_pickle=True) as arrays:
            keys = arrays.files
            if not multiscale:
                fine_key = "x_f_gt" if "x_f_gt" in keys else "x_f"
                keys = [key for key in keys if key in {
                    fine_key, "m_f", "available_mask", "target_mask"
                }]
            self.arrays = {key: arrays[key] for key in keys}
        ...
        if diverse_mask_config is not None:
            if multiscale or any(k in self.arrays for k in ('m_f', 'target_mask')):
                raise ValueError('Diverse masks require single-scale data without embedded observation/target masks')
            if mask_csv is not None or fixed_mask_csv is not None:
                raise ValueError('Choose generated diverse masks or a CSV, not both')
            self.diverse_masks = DiverseMaskSchedule(self.length, diverse_mask_config)
        ...
        if csv_path is not None:
            ...
            # Preserve both single-row and single-column protocol masks.
            masks_flat = np.loadtxt(str(csv_file), delimiter=",", dtype=np.float32, ndmin=2)

            # Infer expected shape from NPZ data
            first = torch.as_tensor(self.arrays[self.x_key][0]).unsqueeze(0)
            _, _, t, h, w = to_bcthw(first).shape
            spatial_cols = h * w
            st_cols = t * h * w
    ...
    def set_epoch(self, epoch: int) -> None:
        if self.diverse_masks is not None:
            self.diverse_masks.set_epoch(epoch)

    def _tensor_at(self, key: str, idx: int) -> torch.Tensor:
        return torch.as_tensor(self.arrays[key][idx]).float()

    def _mask_at(self, key: str, idx: int, target: torch.Tensor) -> torch.Tensor:
        mask = self._tensor_at(key, idx)
        channels = (1, target.shape[0])
        if mask.ndim == 4:
            if mask.shape[0] in channels and mask.shape[1:] == target.shape[1:]:
                return mask
            if mask.shape[-1] in channels and mask.shape[:3] == target.shape[1:]:
                return mask.permute(3, 0, 1, 2).contiguous()
        raise ValueError(f"{key} shape {tuple(mask.shape)} does not match {tuple(target.shape)}")

    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        x_f_gt = to_bcthw(self._tensor_at(self.x_key, idx).unsqueeze(0)).squeeze(0)
        family_id = None
        if self.diverse_masks is not None:
            mask, family_id = self.diverse_masks.sample(idx, tuple(x_f_gt.shape[1:]))
            m_f = torch.from_numpy(mask).unsqueeze(0)
        elif "m_f" in self.arrays:
            m_f = self._mask_at("m_f", idx, x_f_gt)
        ...
        if family_id is not None:
            sample['mask_family'] = torch.tensor(family_id, dtype=torch.long)
        if not self.multiscale:
            for key in ("available_mask", "target_mask"):
                if key in self.arrays:
                    sample[key] = self._mask_at(key, idx, x_f_gt)
            return prepare_single_scale(sample)
