...
def ensure_observed(sample: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    if "x_f_obs" not in sample:
        sample["x_f_obs"] = torch.where(sample["m_f"].bool(), sample["x_f_gt"], 0.0)
    return sample


def prepare_single_scale(sample: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    """Construct visible inputs and supervision independently, with no pooling.

    ``available_mask`` denotes original availability O and ``target_mask`` an
    optional explicit holdout Q. Nonfinite labels are never supervision. Both
    masks may have one channel or one per variable; only M enters the model.
    """
    target = sample["x_f_gt"]
    for key in ("m_f", "available_mask", "target_mask"):
        if key not in sample:
            continue
        mask = sample[key]
        if mask.ndim != target.ndim or mask.shape[-3:] != target.shape[-3:]:
            raise ValueError(f"{key} must match target time and spatial dimensions")
        if not bool(((mask == 0) | (mask == 1)).all()):
            raise ValueError(f"{key} must be binary and finite")
    available = torch.isfinite(target)
    if "available_mask" in sample:
        available = available & sample["available_mask"].bool()
    observed = sample["m_f"].bool() & available
    if "target_mask" in sample:
        requested = sample["target_mask"].bool() & available
        observed = observed & ~requested
        supervised = requested
    else:
        supervised = available & ~observed
    sample["m_f"] = observed.to(target.dtype)
    sample["target_mask"] = supervised.to(target.dtype)
    sample["available_mask"] = available.to(target.dtype)
    sample["x_f_obs"] = torch.where(observed, target, 0.0)
    return sample
