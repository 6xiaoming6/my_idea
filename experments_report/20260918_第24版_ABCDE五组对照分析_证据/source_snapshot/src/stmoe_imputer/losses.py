...
def supervision_mask(
    ...
    """Finite hidden targets Q; natural missing entries are never labels."""
    selected = ~expand_mask_as(mask, target).bool() & torch.isfinite(target)
    if target_mask is not None:
        selected = selected & expand_mask_as(target_mask, target).bool()
    return selected


def _selected_loss(pred: torch.Tensor, target: torch.Tensor, selected: torch.Tensor,
                   loss_type: str) -> torch.Tensor:
    # Index BEFORE subtraction: NaN * 0 is still NaN. An empty indexed sum
    # remains differentiable and gives the defined zero-target fallback.
    pred_values, target_values = pred[selected], target[selected]
    if loss_type == "l1":
        total = F.l1_loss(pred_values, target_values, reduction="sum")
    elif loss_type == "mse":
        total = F.mse_loss(pred_values, target_values, reduction="sum")
    elif loss_type == "smooth_l1":
        total = F.smooth_l1_loss(pred_values, target_values, reduction="sum")
    else:
        raise ValueError(f"Unsupported loss_type: {loss_type}")
    return total / selected.sum().clamp_min(1)


def coe_load_balance_per_step(
    route_probs: torch.Tensor, route_weights: torch.Tensor,
) -> torch.Tensor:
    """Batch-level expert balance for each round of window routing, in FP32.

    Both inputs have shape [B, rounds, experts]. Hard routing supplies its
    actual (possibly Gumbel-sampled) one-hot selections as route_weights; soft
    routing supplies mixture weights. Detach the measured load so gradients
    flow through clean routing probabilities, not the straight-through sample.
    The caller averages rounds to keep the coefficient independent of depth.
    """
    if (
        route_probs.ndim != 3
        or route_probs.shape != route_weights.shape
        or any(size < 1 for size in route_probs.shape)
    ):
        raise ValueError("CoE routing probabilities and weights must share nonempty [B, rounds, experts] shape")
    if route_probs.device != route_weights.device:
        raise ValueError("CoE routing probabilities and weights must be on the same device")
    load = route_weights.detach().float().mean(dim=0)
    importance = route_probs.float().mean(dim=0)
    return route_probs.shape[-1] * (load * importance).sum(dim=-1)


def compute_coe_loss(outputs: dict, batch: dict, cfg: dict) -> tuple[torch.Tensor, dict]:
    loss_cfg = cfg.get("loss", {})
    loss_type = loss_cfg.get("type", "l1")
    mid_weight = float(loss_cfg.get("lambda_coe_mid", 0.0))
    # First-version configs disable this optional follow-up regularizer.
    balance_weight = float(loss_cfg.get("lambda_coe_balance", 0.0))
    z_weight = float(loss_cfg.get("lambda_coe_z", 0.0))
    for name, weight in (("lambda_coe_mid", mid_weight), ("lambda_coe_balance", balance_weight), ("lambda_coe_z", z_weight)):
        if not math.isfinite(weight) or weight < 0:
            raise ValueError(f"loss.{name} must be finite and nonnegative")
    target = batch["x_f_gt"]
    selected = supervision_mask(target, batch["m_f"], batch.get("target_mask"))
    main = _selected_loss(outputs["x_hat_main"], target, selected, loss_type)
    mid = main * 0.0
    coe = outputs["coe"]
    predictions = coe["predictions"][:-1]
    if mid_weight and predictions:
        mid = torch.stack([
            _selected_loss(pred, target, selected, loss_type) for pred in predictions
        ]).mean()
    ...
    if (
        ...
        if probs is None or weights is None:
            ...
            if probs.shape[-1] > 1:
                # A window with no supervised targets has no task-driven route
                # to balance. In particular an empty-Q batch stays exactly zero.
                balance_steps = coe_load_balance_per_step(
                    probs[supervised_samples], weights[supervised_samples]
                )
    balance = balance_steps.mean()
    # Disabling the term removes its graph from the training objective while
    # retaining the raw diagnostic for the no-balance ablation.
    weighted_balance = balance_weight * balance if balance_weight else main.new_zeros((), dtype=torch.float32)
    z = main.new_zeros((), dtype=torch.float32)
    if z_weight and coe.get("use_routed", True) and coe.get("routing_mode") in ("hard", "soft") and bool(supervised_samples.any()):
        logits = coe.get("route_logits")
        if logits is None:
            raise ValueError("Router z-loss requires route_logits")
    ...
    logs = {
        ...
        "l_coe_z": z.detach(), "l_coe_z_weighted": weighted_z.detach(),
        "l_coe_mid": mid.detach(), "supervised_count": selected.sum().detach(),
        "l_coe_balance": balance.detach(),
        "l_coe_balance_weighted": weighted_balance.detach(),
    }
...
def compute_main_stage_loss(
    ...
    if "coe" in outputs:
        return compute_coe_loss(outputs, batch, cfg)
    loss_cfg = cfg["loss"]
