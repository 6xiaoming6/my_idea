...
from lib.utils import get_adjacency_matrix, get_adjacency_matrix_2direction, compute_val_loss, predict_and_save_results, load_graphdata_normY_channel1,load_graphdata_normY_channel1_imputation
...
use_nni = True if training_config['use_nni'] == 1 else False

print(f'use_nni={use_nni}' )

if use_nni:
    params = nni.get_next_parameter()
    num_layers = int(params['num_layers'])
    smooth_layer_num = int(params['smooth_layer_num'])
...
folder_dir = ctime()+'_%s_%s_%.2fMAE_%s_layer%d_head%d_dm%d_channel%d_dir%d_drop%.2f_%.2e_' % (dataset_name,miss_type,miss_rate,model_name, num_layers, nb_head, d_model, encoder_input_size, direction, dropout, learning_rate)
folder_dir += str(random.randint(1,10000))
...
train_loader, train_target_tensor, val_loader, val_target_tensor, test_loader, test_target_tensor, _mean, _std = load_graphdata_normY_channel1_imputation(config,DEVICE, batch_size)
...
def train_main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_index, batch_data in enumerate(train_loader):
            ...
            loss = criterion(outputs[eval_points], labels[eval_points])
    ...
    for epoch in range(epochs, epochs+fine_tune_epochs):
        ...
        for batch_index, batch_data in enumerate(train_loader):
            ...
            loss = criterion(predict_output[eval_points], labels[eval_points])
...
def predict_main(epoch, data_loader, data_target_tensor, _std, _mean, type):
    '''
    在测试集上，测试指定epoch的效果
    :param epoch: int
    ...
    predict_and_save_results(net, data_loader, data_target_tensor, epoch, _std, _mean, params_path, type)
