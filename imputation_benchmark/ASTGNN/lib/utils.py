...
from prepareData import loadImputationData
...
def compute_val_loss(net, val_loader, criterion, sw, epoch):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            encoder_inputs, decoder_inputs, labels,eval_points = batch_data
            eval_points = eval_points.bool()
...
def predict_and_save_results(net, data_loader, data_target_tensor, epoch, _std, _mean, params_path, type):
    '''
    for transformerGCN
    :param net: nn.Module
    ...
    with torch.no_grad():
        ...
        data_target_tensor = data_target_tensor*_std + _mean
        ...
        for batch_index, batch_data in enumerate(data_loader):
            ...
            eval_points_all.append(eval_points.detach().cpu().numpy())
            if batch_index % 100 == 0:
...
def load_graphdata_normY_channel1_imputation(config, DEVICE, batch_size, shuffle=True, percent=1.0):
    '''
    将x,y都处理成归一化到[-1,1]之前的数据;
    每个样本同时包含所有监测点的数据，所以本函数构造的数据输入时空序列预测模型；
    该函数会把hour, day, week的时间串起来；
    ...
    x_trains, te_trains, cond_trains, ob_trains,x_vals, te_vals, cond_vals, ob_vals,\
    x_tests, te_tests, cond_tests, ob_tests,\
    data_mean, data_std = loadImputationData(config)

    # 统一对x进行归一化
    x_trains = (x_trains - data_mean)/data_std
    
    x_vals = (x_vals - data_mean)/data_std
    
    x_tests = (x_tests - data_mean)/data_std
    

    train_target = x_trains.copy()[:,:,0,:]
    eval_trains = ob_trains[:,:,0,:] - cond_trains[:,:,0,:]
    train_target[eval_trains == 0] = 0

    test_target = x_tests.copy()[:,:,0,:]
    eval_tests = ob_tests[:,:,0,:] - cond_tests[:,:,0,:]
    test_target[eval_tests == 0] = 0

    val_target = x_vals.copy()[:,:,0,:]
    eval_vals = ob_vals[:,:,0,:] - cond_vals[:,:,0,:]
    val_target[eval_vals == 0] = 0


    x_trains[cond_trains == 0] = 0
    x_vals[cond_vals == 0] = 0
    x_tests[cond_tests == 0] = 0

    #  ------- train_loader -------
    train_decoder_input_start = x_trains[:, : , 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    train_decoder_input_start = np.squeeze(train_decoder_input_start, 2)  # (B,N,T(1))
    train_decoder_input = np.concatenate((train_decoder_input_start, train_target[:, :, :-1]), axis=2)  # (B, N, T)

    train_x_tensor = torch.from_numpy(x_trains).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    train_decoder_input_tensor = torch.from_numpy(train_decoder_input).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    train_target_tensor = torch.from_numpy(train_target).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    eval_trains = torch.from_numpy(eval_trains).type(torch.LongTensor).to(DEVICE)  # (B, N, T)

    train_dataset = torch.utils.data.TensorDataset(train_x_tensor, train_decoder_input_tensor, train_target_tensor,eval_trains)

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)

    #  ------- val_loader -------
    val_decoder_input_start = x_vals[:, :, 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    val_decoder_input_start = np.squeeze(val_decoder_input_start, 2)  # (B,N,T(1))
    val_decoder_input = np.concatenate((val_decoder_input_start, val_target[:, :, :-1]), axis=2)  # (B, N, T)

    val_x_tensor = torch.from_numpy(x_vals).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    val_decoder_input_tensor = torch.from_numpy(val_decoder_input).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    val_target_tensor = torch.from_numpy(val_target).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    eval_vals = torch.from_numpy(eval_vals).type(torch.LongTensor).to(DEVICE)  # (B, N, T)

    val_dataset = torch.utils.data.TensorDataset(val_x_tensor, val_decoder_input_tensor, val_target_tensor,eval_vals)

    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size)

    #  ------- test_loader -------
    test_decoder_input_start = x_tests[:, :, 0:1, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    test_decoder_input_start = np.squeeze(test_decoder_input_start, 2)  # (B,N,T(1))
    test_decoder_input = np.concatenate((test_decoder_input_start, test_target[:, :, :-1]), axis=2)  # (B, N, T)

    test_x_tensor = torch.from_numpy(x_tests).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    test_decoder_input_tensor = torch.from_numpy(test_decoder_input).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)
    test_target_tensor = torch.from_numpy(test_target).type(torch.FloatTensor).to(DEVICE)  # (B, N, T)

    eval_tests = torch.from_numpy(eval_tests).type(torch.LongTensor).to(DEVICE)  # (B, N, T)

    test_dataset = torch.utils.data.TensorDataset(test_x_tensor, test_decoder_input_tensor, test_target_tensor,eval_tests)

    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size)

    # print
    print('train:', train_x_tensor.size(), train_decoder_input_tensor.size(), train_target_tensor.size(),eval_trains.size())
    print('val:', val_x_tensor.size(), val_decoder_input_tensor.size(), val_target_tensor.size(),eval_vals.size())
    print('test:', test_x_tensor.size(), test_decoder_input_tensor.size(), test_target_tensor.size(),eval_tests.size())

    return train_loader, train_target_tensor, val_loader, val_target_tensor, test_loader, test_target_tensor, data_mean, data_std
