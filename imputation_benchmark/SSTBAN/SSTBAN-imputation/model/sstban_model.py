...
class SSTBAN(nn.Module):
    ...
    def __init__(self, args, bn_decay):
        ...
        self.STAttBlock_1 = nn.ModuleList([STAttBlock(K, d,self.num_his,self.num_of_vertices,set_dim, bn_decay) for _ in range(L)])
        self.STAttBlock_self = nn.ModuleList([STAttBlock_self(K, d, self.num_his, self.num_of_vertices, set_dim, bn_decay) for _ in range(1)])
        self.STAttBlock_2 = nn.ModuleList([STAttBlock(K, d,self.num_pred,self.num_of_vertices,set_dim,bn_decay) for _ in range(L)])
    ...
    def forward(self, X, TE,mode,mask=None,ob_mask=None):
        ...
        if mask is not None:
            mask = mask.repeat(1,1,1,X.shape[-1])

        if mode =='train':
            batch_mask_matrix = np.random.rand(X.shape[0],int(X.shape[1]/T_miss_len),X.shape[2],X.shape[3])>self.node_miss_rate
            batch_mask_matrix = torch.from_numpy(np.repeat(batch_mask_matrix,T_miss_len,axis=1)).to(torch.float32).cuda()
            
            X_miss = X.detach().clone()
            if mask is not None:
                batch_mask_matrix *= mask
            X_miss[batch_mask_matrix==0] = 0
