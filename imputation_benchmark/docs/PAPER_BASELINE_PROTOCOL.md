...
python imputation_benchmark/scripts/launch/run_all_baseline_train_2gpu.py \
  --gpus 0 \
  --masks fixed
...
intervals are controlled by JSON files under `imputation_benchmark/configs/policies/`.
...
python imputation_benchmark/scripts/launch/run_all_baseline_train_2gpu.py \
  ...
  --masks fixed \
  --rates 0.2 \
  --policy-json imputation_benchmark/configs/policies/baseline_5epoch_test.json
```
...
python imputation_benchmark/scripts/launch/run_all_baseline_train_2gpu.py \
  --gpus 0 1 \
  --resume-run imputation_benchmark/artifacts/runs/paper/<run_id> \
  --skip-prepare
```
...
`imputation_benchmark/artifacts/runs/paper/<run_id>/launcher_logs/`. Standardized model
...
For the lightweight all-overview-model contract test, use:

```bash
python imputation_benchmark/scripts/launch/run_overview_baseline_1epoch.py --gpu 0
```

This is the maintained smoke/integration entry point. It replaces the former
collection of per-model `smoke_tests/run_*_smoke.py` forwarding scripts.
