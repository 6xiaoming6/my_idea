...
class Trainer(object):
    ...
    def train_epoch(self, epoch):
        self.model.train()
        total_loss = 0
        for batch_idx, (data, target,gtmask) in enumerate(self.train_loader):
            data = data[..., :self.args.input_dim]
    ...
    @staticmethod
    def test(model, args, data_loader, scaler, logger,use_nni=False, path=None):
        if path != None:
        ...
        with torch.no_grad():
            for batch_idx, (data, target,gtmask) in enumerate(data_loader):
                data = data[..., :args.input_dim]
                label = target[..., :args.output_dim]
                output = model(data, target, teacher_forcing_ratio=0)
                y_true.append(label[gtmask].view(-1))
                y_pred.append(output[gtmask].view(-1))
