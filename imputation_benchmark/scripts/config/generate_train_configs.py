...
MODELS = ROOT / "baselines"
OUT = ROOT / "configs" / "training"
ADAPTED_ROOT = ROOT / "data" / "adapted"
DATASETS = {
    "TaxiBJ": (32, 32, 2, 12),
    "BikeNYC": (24, 12, 2, 12),
    "CHAP": (32, 32, 1, 7),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--adapted-root", default=str(ADAPTED_ROOT))
    parser.add_argument("--output-root", default=str(OUT))
    parser.add_argument(
        "--policy-json", default=str(ROOT / "configs" / "policies" / "baseline_paper.json"),
...
def adapted_dir(dataset: str, mask: str, rate: float, channel: str) -> Path:
    return ADAPTED_ROOT / dataset / f"{mask}_{label(rate)}" / f"channel_{channel}"
...
def main() -> None:
    global ADAPTED_ROOT, OUT
    args = parse_args()
    ADAPTED_ROOT = Path(args.adapted_root).expanduser()
    if not ADAPTED_ROOT.is_absolute():
        ADAPTED_ROOT = (ROOT.parent / ADAPTED_ROOT).resolve()
    ...
    specs = [
        ("AGCRN", MODELS / "AGCRN", "configurations/PEMS04.conf", {
            "data": {"dataset": dataset, "data_prefix": relative_split_data(MODELS / "AGCRN", dataset, mask, rate, channel),
        ...
        ("IGNNK", MODELS / "IGNNK", "configurations/PEMS04.conf", {
            "file": {"data_prefix": relative_split_data(MODELS / "IGNNK", dataset, mask, rate, channel),
                     "distance_df_filename": relative_split_data(MODELS / "IGNNK", dataset, mask, rate, channel) + "/grid_edges.csv",
        ...
        ("mTAN", MODELS / "mTAN", "configurations/PEMS04_SC-TC_0.5.conf", {
            "Data": {"data_prefix": relative_split_data(MODELS / "mTAN", dataset, mask, rate, channel),
                     "save_prefix": f"./experiments/{dataset}_{mask}_{rate_text}_channel_{channel}",
        ...
        ("GAIN", MODELS / "GAIN", "configurations/PEMS04.conf", {
            "file": {"data_prefix": relative_split_data(MODELS / "GAIN", dataset, mask, rate, channel),
                     "save_prefix": f"./experiments/{dataset}_{mask}_{rate_text}_channel_{channel}"},
        ...
        ("E2GAN", MODELS / "E2GAN", "configurations/PEMS04.conf", {
            "file": {"data_prefix": relative_split_data(MODELS / "E2GAN", dataset, mask, rate, channel),
                     "save_prefix": f"./experiments/{dataset}_{mask}_{rate_text}_channel_{channel}"},
        ...
        ("ASTGNN", MODELS / "ASTGNN", "configurations/PEMS04_SR-TC_70.conf", {
            "Data": {"adj_filename": relative_data(MODELS / "ASTGNN", dataset, mask, rate, channel) + "/grid_edges.csv",
                     "graph_signal_matrix_filename": relative_data(MODELS / "ASTGNN", dataset, mask, rate, channel),
        ...
        ("SSTBAN", MODELS / "SSTBAN" / "SSTBAN-imputation", "configurations/PEMS04.conf", {
            "Data": {"dataset_name": dataset,
                     "data_prefix": relative_data(MODELS / "SSTBAN" / "SSTBAN-imputation", dataset, mask, rate, channel),
    ...
    gcastn = MODELS / "GCASTN" / "GCASTN-main" / "code_data_paper_632" / "GCASTN"
    ...
    pristi_dir = MODELS / "PriSTI" / "PriSTI-main"
    ...
    impute_dir = MODELS / "imputeformer"
    ...
    for model in ("SAITS", "GRIN", "STCPA", "STAMImputer", "PAST", "MeanFill", "HistoricalAverage"):
        ...
        payload = {
            ...
            "output": {"checkpoint": str(
                ROOT / "artifacts" / "native_experiments" / f"{dataset}_{model}_{mask}_{rate_text}_channel_{channel}" / "best_model.pth"
            )},
        }
