...
These scripts train on the complete adapted dataset. Model architecture and
training hyperparameters come from each baseline's original template.

Example:

```bash
python imputation_benchmark/scripts/train/train_astgnn.py \
...
generates a config under `imputation_benchmark/configs/training`. Reuse them:
...
```bash
python imputation_benchmark/scripts/train/compact_existing_logs.py \
  --output-root outputs
```
...
`imputation_benchmark/scripts/launch/run_all_baseline_train_2gpu.py`. The exact fairness
...
Use `--policy-json imputation_benchmark/configs/policies/baseline_5epoch_test.json` for
...
## Clean one-epoch integration test

The former per-model smoke dispatch scripts have been replaced by one entry:

```bash
python imputation_benchmark/scripts/launch/run_overview_baseline_1epoch.py --gpu 0
```

It includes only the 13 implemented methods in the survey overview, runs jobs
sequentially, and uses four source windows per train/validation/test split by
default. Every trainable model is required to report exactly epoch 1,
validation epoch 1, and finite test MAE/RMSE. MeanFill and HistoricalAverage
perform one deterministic fit/validation/test pass; LATC performs one native
optimization iteration. Test data and configs are isolated under
`data/testing/overview_1epoch` and `configs/testing/overview_1epoch`, so formal configs are not
overwritten. Increase `--windows` only when a larger integration test is
needed.
