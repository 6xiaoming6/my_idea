...
class BaselineLogAdapter:
    ...
    def finalize(self, status: str, returncode: int, total_time: float) -> None:
        ...
        data_root = Path(getattr(self.args, "data_root", self.project / "imputation_benchmark" / "data" / "adapted"))
        if not data_root.is_absolute():
            data_root = (self.project / data_root).resolve()
        manifest_path = (data_root / self.args.dataset /
                         f"{self.args.mask}_{format(self.args.rate, 'g')}" /
