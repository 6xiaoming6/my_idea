...
def parse_args(model: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-root", default=str(BENCH / "data" / "adapted"),
        help="Prepared adapter-data root (the one-epoch tester uses an isolated small-data root)",
    )
    parser.add_argument(
        "--config-root", default=str(BENCH / "configs" / "training"),
        help="Generated native-config root",
    )
    parser.add_argument(
        "--policy-json", default=str(BENCH / "configs" / "policies" / "baseline_paper.json"),
...
def config(args: argparse.Namespace, model: str, suffix: str = "conf") -> str:
    ...
    return str(root / args.dataset / name)
...
def adapted_data(args: argparse.Namespace, split: bool = False) -> Path:
    ...
    if not root.is_absolute():
        root = (PROJECT / root).resolve()
    path = root / args.dataset / f"{args.mask}_{rate_label(args.rate)}" / f"channel_{args.channel}"
...
def prepare(args: argparse.Namespace) -> None:
    ...
    subprocess.run([
        args.python, str(BENCH / "scripts" / "data" / "prepare_grid_dataset.py"), "--dataset", args.dataset,
        "--mask", args.mask, "--rate", rate_label(args.rate), "--channel", args.channel,
        "--legacy-stream", "--output-root", str(Path(args.data_root).expanduser()),
    ], cwd=PROJECT, check=True)
    subprocess.run([
        args.python, str(BENCH / "scripts" / "config" / "generate_train_configs.py"), "--dataset", args.dataset,
        "--mask", args.mask, "--rate", rate_label(args.rate), "--channel", args.channel,
        "--seed", str(args.seed), "--policy-json", args.policy_json,
        "--adapted-root", str(Path(args.data_root).expanduser()),
        "--output-root", str(Path(args.config_root).expanduser()),
    ], cwd=PROJECT, check=True)
...
def _checkpoint_roots(model: str, cwd: Path) -> tuple[Path, ...]:
    ...
    roots = {
        ...
        "ASTGNN": (BENCH / "artifacts" / "native_experiments",),
        ...
        "SAITS": (BENCH / "artifacts" / "native_experiments",),
        "GRIN": (BENCH / "artifacts" / "native_experiments",),
        "STCPA": (BENCH / "artifacts" / "native_experiments",),
        "STAMImputer": (BENCH / "artifacts" / "native_experiments",),
        "PAST": (BENCH / "artifacts" / "native_experiments",),
    }
