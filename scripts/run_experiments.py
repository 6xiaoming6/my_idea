...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--model_config_dir",
        default=None,
        help="Optional directory containing dataset model overrides, e.g. configs/v10-single.",
    )
    parser.add_argument(
        "--cpu-threads",
        "--cpu_threads",
...
def _load_model_patches(path_text: str | None, datasets: tuple[str, ...]) -> tuple[str | None, dict[str, dict]]:
    ...
    patches = {}
    for dataset in datasets:
        path = root / DATASETS[dataset].model_config_name
        if not path.is_file():
            raise FileNotFoundError(f"Model override not found for {dataset}: {path}")
        patch = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(patch, dict):
            raise ValueError(f"Model override must be a JSON object: {path}")
        patches[dataset] = patch
    return str(root), patches
