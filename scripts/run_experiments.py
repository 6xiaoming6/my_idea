...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional directory containing taxibj.json, bikenyc.json, and chap.json. "
            "The selected model patch is merged before the training policy and mask patch."
        ),
    )
    parser.add_argument(
        "--cpu-threads",
...
def _load_model_configs(
    ...
    for dataset in datasets:
        path = directory / DATASETS[dataset].model_config_name
        if not path.is_file():
...
def _run_dataset_combo(
    ...
    base_override = _deep_update(model_override, training_override)
...
def main() -> None:
    ...
    model_config_dir, model_patches = _load_model_configs(args.model_config_dir, dataset_names)
    ...
    print(f"model configs: {model_config_dir or 'dataset defaults (main)'}")
