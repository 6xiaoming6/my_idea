...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--model_config_dir",
        default=None,
        help="Optional directory containing dataset model patches (taxibj.json, bikenyc.json, chap.json).",
    )
    parser.add_argument(
        "--cpu-threads",
        "--cpu_threads",
...
def _load_model_configs(
    ...
    for dataset in datasets:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"Dataset model config not found: {path}")
        patch = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(patch, dict):
            raise ValueError(f"Dataset model config must contain a JSON object: {path}")
