#!/usr/bin/env python3
"""Run the gated V14 RLB exploration stages sequentially on one GPU."""
...
CONFIGS = {
    "R00": CONFIG_ROOT / "R00_legacy_hard.json",
    "R01": CONFIG_ROOT / "R01_switch_topk_1e-4.json",
    "R02": CONFIG_ROOT / "R02_switch_topk_1e-3.json",
    "R03": CONFIG_ROOT / "R03_switch_topk_1e-2.json",
}
SCREEN_POINTS = (
    ("TaxiBJ", "fixed", "0.4"),
    ("BikeNYC", "random", "0.2"),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("R01", "R02", "R03"),
        default=None,
        help="Required after screen; select from validation results only.",
    )
    ...
    parser.add_argument(
        ...
        help="Model seeds used only by --phase multiseed.",
    )
...
def main() -> None:
    ...
    for index, (candidate, config_path, dataset, pattern, rate, seed) in enumerate(
        ...
        expected_epochs = (
            args.epochs if args.epochs is not None else FULL_EPOCHS[dataset]
        )
        if (
