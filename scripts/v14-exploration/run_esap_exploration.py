#!/usr/bin/env python3
"""Run the long-form V14 ESAP scale-pathway exploration on one GPU."""
...
# Measured medians from the completed CSAS full runs on this workstation.
...
def _completed(
    ...
    for test_log in sorted(root.glob("*/logs/test.log"), reverse=True):
        ...
        if (
            ...
            and cfg.get("experiment") == expected_experiment
        ):
            return True
    return False
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--phase",
        choices=("screen", "multiseed", "all24"),
        default="screen",
    )
    parser.add_argument(
        "--candidates",
        ...
        help="Screen defaults to all 18 candidates; later phases require exactly one.",
    )
...
def main() -> None:
    args = parse_args()
    if len(CONFIGS) != 18:
        raise RuntimeError(f"Expected 18 ESAP configs, found {len(CONFIGS)}")
    ...
    for job in jobs:
        ...
        if args.rerun_completed or not _completed(
            dataset, pattern, rate, path, seed, expected_epochs
        ):
            pending.append(job)
    estimated_hours = sum(MEASURED_HOURS[job[2]] for job in pending)
    print(
        f"ESAP {args.phase}: {len(pending)}/{len(jobs)} pending; "
