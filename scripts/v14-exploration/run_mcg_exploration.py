#!/usr/bin/env python3
"""Run the gated V14 monotonic-consistency-gate exploration on one GPU."""
...
CONFIG_ROOT = ROOT / "configs/v14-exploration/gate"
CONFIGS = {
    "M01": CONFIG_ROOT / "M01_monotonic_gain_0.5.json",
    "M02": CONFIG_ROOT / "M02_monotonic_gain_1.0.json",
    "M03": CONFIG_ROOT / "M03_monotonic_gain_2.0.json",
}
# Includes the two known fixed@0.8 counterexamples to the observed-gain
# correlation, one low-rate task, and all three datasets.
SCREEN_POINTS = (
    ("TaxiBJ", "fixed", "0.8"),
    ("BikeNYC", "random", "0.2"),
    ("CHAP", "random", "0.4"),
...
def _completed(
    ...
    for test_log in sorted(root.glob("*/logs/test.log"), reverse=True):
        ...
        if not all(path.is_file() for path in (config_file, metrics_file, checkpoint)):
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Required after screen; choose using validation metrics only.",
    )
...
def _jobs(args: argparse.Namespace) -> list[tuple[str, Path, str, str, str, int]]:
    ...
    if args.candidate is None:
        raise ValueError(
            ...
            "select it from validation metrics, never test metrics"
        )
    if args.phase == "core6":
...
def main() -> None:
    ...
    for index, (candidate, config_path, dataset, pattern, rate, seed) in enumerate(
        ...
        if (
            ...
            and _completed(
                dataset, pattern, rate, config_path, seed, expected_epochs
            )
        ):
