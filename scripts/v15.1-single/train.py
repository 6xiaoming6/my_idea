#!/usr/bin/env python3
"""Train one V15.1 dataset/mask/rate combination with isolated outputs."""
...
DATASETS = {
    "TaxiBJ": {
        ...
        "model": "configs/v15.1-single/taxibj.json",
    ...
    "BikeNYC": {
        ...
        "model": "configs/v15.1-single/bikenyc.json",
    ...
    "CHAP": {
        ...
        "model": "configs/v15.1-single/chap.json",
...
ABLATIONS = {
    "fixed_acceptance": "fixed_acceptance.json",
    "no_acceptance_loss": "no_acceptance_loss.json",
    "no_scale_guidance": "no_scale_guidance.json",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-name",
        default=None,
        help="Optional output variant name; quick epoch overrides default to debug output.",
    )
...
def main() -> None:
    ...
    if args.ablation != "none":
        patch = _deep_update(
            patch,
            _load(ROOT / "configs" / "v15.1-single" / "ablations" / ABLATIONS[args.ablation]),
        )
    ...
    if args.run_name:
        ...
        run_name = f"ablation_v15_1_{args.ablation}"
    elif args.epochs is not None or args.training_policy:
        run_name = f"debug_v15_1_{args.epochs or 'policy'}epoch"
    else:
        run_name = "full"
    with tempfile.TemporaryDirectory(prefix="v15_1_single_") as directory:
