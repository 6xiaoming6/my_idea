#!/usr/bin/env python3
"""Train one V16 dataset/mask/rate experiment with an isolated output tree."""
...
DATASETS = {
    "TaxiBJ": {
        ...
        "model": "configs/v16-single/taxibj.json",
    ...
    "BikeNYC": {
        ...
        "model": "configs/v16-single/bikenyc.json",
    ...
    "CHAP": {
        ...
        "model": "configs/v16-single/chap.json",
...
ABLATIONS = {
    "no_teacher_anchor": "no_teacher_anchor.json",
    "fixed_alpha": "fixed_alpha.json",
    "original_9d_condition": "original_9d_condition.json",
    "binary_acceptance": "binary_acceptance.json",
}
...
def _ensure_masks(args: argparse.Namespace, spec: dict, env: dict[str, str]) -> Path:
    mask_family = (
        f"{args.mask}_mask"
        if args.mask_seed == 42
        else f"{args.mask}_mask_seed{args.mask_seed}"
    )
    mask_dir = ROOT / spec["mask_root"] / mask_family / args.rate
...
def main() -> None:
    ...
    if args.training_policy:
        ...
        patch = _deep_update(patch, {"experiment_policy": {"name": policy.get("name", policy_path.stem), "source": str(policy_path)}})
    if args.epochs is not None:
        patch = _deep_update(patch, {
            "train": {"epochs": args.epochs, "val_epoch": 1, "early_stopping": {"enabled": False}},
            "model": {"v16": {"warmup_epochs": min(12, max(0, args.epochs // 5))}},
        })
    if args.ablation != "none":
        patch = _deep_update(
            patch,
            _load(ROOT / "configs" / "v16-single" / "ablations" / ABLATIONS[args.ablation]),
        )
    ...
    patch = _deep_update(patch, {
        ...
        "teacher": {"seed": args.teacher_seed},
        "data": {"mask": {
            "pattern": args.mask,
    ...
    if args.run_name:
        ...
        run_name = f"ablation_v16_{args.ablation}"
    elif args.epochs is not None or args.training_policy:
        run_name = f"debug_v16_{args.epochs or 'policy'}epoch"
    else:
        run_name = "full"
    with tempfile.TemporaryDirectory(prefix="v16_single_") as directory:
