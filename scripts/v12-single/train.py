#!/usr/bin/env python3
"""Run one v12 frequency multi-resolution MoE experiment."""
...
ABLATIONS = ("main_fallback", "low_only", "high_only", "eta_fixed_0.05")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--dataset", choices=tuple(DATASET_CONFIG_NAMES), required=True)
...
def _run(args: argparse.Namespace, model_config_dir: str, run_name: str) -> None:
    ...
    if args.quick is not None:
        policy = f"configs/v12-single/policies/quick_{args.quick}epoch.json"
    command = [
        sys.executable,
...
def main() -> None:
    ...
    if args.ablation is None:
        _run(args, "configs/v12-single", "full")
        return
    config_name = DATASET_CONFIG_NAMES[args.dataset]
    base_path = ROOT / "configs" / "v12-single" / config_name
    ablation_path = ROOT / "configs" / "v12-single" / "ablations" / f"{args.ablation}.json"
    combined = _deep_update(
        json.loads(base_path.read_text(encoding="utf-8")),
        json.loads(ablation_path.read_text(encoding="utf-8")),
    )
    with tempfile.TemporaryDirectory(prefix="v12_ablation_") as directory:
        model_dir = Path(directory)
        (model_dir / config_name).write_text(
            json.dumps(combined, indent=2, ensure_ascii=False), encoding="utf-8"
        )
        _run(args, str(model_dir), f"ablation_v12_{args.ablation}")
