#!/usr/bin/env python3
"""Train one V19 dataset/mask/rate combination in a single stage."""
...
DATASETS = {
    "TaxiBJ": {
        ...
        "model": "configs/v19-single/taxibj.json",
    ...
    "BikeNYC": {
        ...
        "model": "configs/v19-single/bikenyc.json",
    ...
    "CHAP": {
        ...
        "model": "configs/v19-single/chap.json",
...
def _python(conda_env: str, *arguments: str) -> list[str]:
    ...
    if conda is None:
        raise FileNotFoundError(
            "conda is unavailable; activate the requested environment first"
        )
    return [
        conda,
        "run",
...
def _ensure_masks(
    ...
    expected = tuple(
        mask_dir / f"{split}.csv" for split in ("train", "val", "test")
    )
...
def main() -> None:
    ...
    for key in ("base", "model", "train", "val", "test"):
        ...
        if not path.is_file() and not (
            args.dry_run and key in {"train", "val", "test"}
        ):
        ...
        patch,
        {
            ...
            "data": {
                "mask": {
                    ...
                    "train_csv": str(
                        (mask_dir / "train.csv").relative_to(ROOT)
                    ),
                    "val_csv": str(
                        (mask_dir / "val.csv").relative_to(ROOT)
                    ),
                    "test_csv": str(
                        (mask_dir / "test.csv").relative_to(ROOT)
                    ),
                }
            },
        },
    ...
    run_name = (
        ...
        else f"debug_v19_{args.epochs}epoch"
    )
    with tempfile.TemporaryDirectory(prefix="v19_single_") as directory:
