#!/usr/bin/env python3
"""Run the free-routing and fixed-chain matrix on two mask protocols."""
...
def check_gpu_idle(gpu: int) -> None:
    ...
    if processes.stdout.strip(): raise RuntimeError('A GPU already has a compute process; dual-mask queue was not started:\n' + processes.stdout)
...
def main() -> int:
    ...
    policy=ROOT/'configs/v24/dual_mask_experiments.json'; cmd=[sys.executable,'-u',str(ROOT/'scripts/v24/run_experiments.py'),'--config',str(policy),'--study','coe_dual_mask','--gpu',str(a.gpu)]
    if a.dry_run or a.summary_only: return subprocess.call(cmd+(['--dry-run'] if a.dry_run else ['--summary-only']),cwd=ROOT)
    ...
    with (output/'.single_gpu_queue.lock').open('a') as lock:
        ...
        except BlockingIOError: raise RuntimeError('Dual-mask queue already running') from None
        check_gpu_idle(a.gpu); return subprocess.call(cmd,cwd=ROOT)
