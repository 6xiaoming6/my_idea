#!/usr/bin/env python3
"""Run the eight CoE mechanism experiments sequentially on one GPU."""
...
def check_gpu_idle(gpu: int) -> None:
    ...
    if processes.stdout.strip():
        raise RuntimeError("A GPU already has a compute process; mechanism1 queue was not started:\n" + processes.stdout)
...
def main() -> int:
    ...
    policy = ROOT / "configs/v24/mechanism1_experiments.json"
    command = [sys.executable, "-u", str(ROOT / "scripts/v24/run_experiments.py"), "--config", str(policy), "--study", "coe_mechanism1", "--gpu", str(args.gpu)]
    ...
    with (output / ".single_gpu_queue.lock").open("a") as lock:
        try:
            ...
            raise RuntimeError("Mechanism1 queue already running") from None
