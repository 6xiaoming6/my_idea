#!/usr/bin/env python3
"""Run main soft-warmup CoE and paired mechanism ablations sequentially."""
...
def check_gpu_idle(gpu: int) -> None:
    ...
    if processes.stdout.strip():
        raise RuntimeError("A GPU already has a compute process; CoE validation queue was not started:\n" + processes.stdout)
...
def main() -> int:
    ...
    args = p.parse_args()
    policy = ROOT / "configs/v24/coe_validation_experiments.json"
    command = [sys.executable, "-u", str(ROOT / "scripts/v24/run_experiments.py"),
               "--config", str(policy), "--study", "coe_validation", "--gpu", str(args.gpu)]
    ...
    with (output / ".single_gpu_queue.lock").open("a") as lock:
        try:
            ...
            raise RuntimeError("CoE validation queue already running") from None
